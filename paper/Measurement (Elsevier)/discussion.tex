...
small number of recordings ($36$) also produces wide variation from one recording
to the next. Scores are therefore reported per held-out recording and summarised
with intervals clustered on that unit, giving intervals of roughly $\pm12$ points;
summarising instead over repeated re-partitionings of the same recordings would
have produced intervals about three times narrower without supplying any
additional evidence about the machine. Those intervals capture
recording-to-recording variation only. The influence quantities that would
dominate a real deployment---a different machine, a different physical specimen of
the same fault, a re-installation of the sensor~\cite{staszewski2025mems}, a
different handset~\cite{capuano2023smartphone}---do not vary anywhere in this
dataset, so none is represented, and the interval must not be read as a
measurement-uncertainty budget in the sense of the GUM. The
