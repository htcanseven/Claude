...
reshuffle the fold composition. The mean accuracy is reported together with its
Type~A expanded uncertainty~\cite{jcgm2008gum}, obtained by treating the five
per-seed mean accuracies as repeated measurements and applying a coverage factor
$k=t_{0.975,\,4}\approx2.78$; the fold-to-fold standard deviation is quoted
separately as a reproducibility spread. For the headline configuration
comparisons, a fixed recording-level fold partition was used so that
configurations are evaluated on identical held-out recordings, enabling a paired
comparison; significance was assessed with a paired $t$-test and a Wilcoxon
signed-rank test on the per-fold differences. This expanded uncertainty reflects
the reproducibility of the evaluation on the available data and does not capture
the additional, unquantified uncertainty arising from the single-machine scope
(Section~\ref{subsec:limits}). Confusion matrices were aggregated from the pooled
