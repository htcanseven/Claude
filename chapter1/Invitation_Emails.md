...
Chapter 1 and the annotated table of contents are not being circulated until the
structure is settled, so each email carries its own description of the invitee's part
and offers a conversation instead. That is the better trade anyway: a call recruits
more reliably than an attachment, and it lets the invitee ask what the commitment
really is.

When the documents do go out, delete the Status column from the contributor table
...
We are preparing a proposal to Wiley-IEEE Press for a new book on high-speed electrical machines, and we would like you to contribute to it.
