...
actually requires, namely recognizing the elementary fault mechanisms of a
compound fault whose exact combination was never observed during model
development. This paper evaluates that ability on a public multimodal dataset
of a 2.2 kW three phase induction motor containing 24 diagnostic states,
including nine compound faults that pair a mechanical bearing defect with a
second mechanical, electrical, or electromagnetic mechanism, under twelve
speed and load conditions. The problem is formulated at the constituent
level, where nine elementary mechanisms are detected independently from
curated features that are specific to each mechanism and drawn from the
vibration and current domains, so that unseen combinations remain
expressible. A crossed protocol separates three levels of compound context
exposure, namely recombination, one sided withholding, and two sided zero
shot withholding, from operating condition novelty, using run level and
severity aware splits with all calibration confined to training data.
Although closed set accuracy over the 24 diagnostic states reaches 0.913,
exact recovery of unseen constituent sets remains between 0.083 and 0.157 for
the locked linear probe. Mechanism resolved controls show that these failures
are strongly heterogeneous. Dynamic eccentricity is almost perfectly
discriminable in isolation (AUROC 0.9995 and $F_1=0.923$), yet its compound
recall falls to 0.017 and its compound $F_1$ to 0.028. Static eccentricity
retains substantial discriminative information but shows marked sensitivity
to classifier family, whereas winding is already less specific in isolation
and remains difficult across classifier families. The primary interaction
between context and condition is positive but becomes unresolved after
matching total training set cardinality. Cross partner transfer is also
heterogeneous and includes one statistically supported reversal of score
orientation. These results show that constituent level evaluation exposes
mechanism specific generalization failures that closed set accuracy alone
conceals.
...
unplanned failures propagate directly into downtime and safety risk, which
has motivated decades of work on condition monitoring for electrical
machines \cite{nandi2005review,bellini2008advances}. Data driven condition
monitoring has consequently produced a large literature whose reported
accuracies now cluster near ceiling on standard laboratory benchmarks
\cite{zhao2020benchmark}. Deployment experience is harder to reconcile with
those numbers. A model commissioned at one operating point degrades at
another, which is the problem addressed by a large body of work on domain
adaptation and domain generalization \cite{han2025,domainshift2025}, and
faults in service rarely arrive one at a time. A bearing defect that develops
...
Conventional closed set formulations cannot even express this situation,
because a previously unseen combination $A{+}B$ constitutes an unseen class.
The compound fault literature therefore moved toward decoupling formulations
that predict elementary mechanisms rather than combinations
\cite{huang2019ddcnn,ltssbow2024,hemtan2024}, and, more recently, toward zero
shot settings in which compound states must be recognized from single fault
training data alone \cite{eswa2022,eswa2023,song2025gzsl}. As
Section~\ref{sec:related} reviews, however, that literature has evaluated
almost exclusively \emph{homogeneous} compounds, meaning bearing and gear
pairings whose constituents are all observable in the same vibration channel.
Its reported accuracies of roughly 75\% to 87\%
\cite{eswa2022,eswa2023,song2025gzsl,appac2025} are not directly comparable
with a leakage safe crossed evaluation at the run level, because many
representative studies use steady operating conditions together with
evaluation protocols defined at the sample or segment level, a practice that
a parallel literature on data leakage has shown to inflate reported
performance \cite{wheat2024,gama2025,phme2025}.
...
states under twelve speed and load conditions, with synchronized triaxial
vibration, three phase currents, torque, and a key phase tachometer. Its nine
compound faults include combinations of two bearing defects and combinations
that pair a bearing defect with broken rotor bars, static or dynamic
eccentricity, or a stator winding short circuit. Their constituent signatures
span evidence in both the vibration and current domains, which provides a
heterogeneous multimodal setting for constituent level generalization. To our
knowledge, neither this cross modal compound regime nor this dataset has
previously received a constituent level evaluation.

The study is designed so that every claim is bounded by an explicit control.
The formulation is at the constituent level, where nine elementary mechanisms
are detected independently by regularized linear probes operating on curated
families of physical signatures that are specific to each mechanism and
derived from angle resampled order spectra, envelope order descriptors
\cite{randall2011tutorial}, and phase current sequence measures
\cite{thomson2001mcsa}. A crossed protocol varies compound context
exposure over three regimes, namely recombination, one sided withholding, and
two sided zero shot withholding, and crosses each with seen and unseen
operating conditions, under run level and severity aware splits with all
preprocessing, calibration, and threshold selection confined to training
data. The principal constituent level metrics are accompanied by input
independent constant predictor references, so that scores driven by
prevalence cannot masquerade as diagnostic skill. Oracle detectability
analyses are paired with recording sensitivity and same date controls, so
that separability is not silently attributed to fault mechanisms. Paired
bootstrap inference clustered by run, supplemented by a cluster sensitivity
analysis at the composition level for the interaction between context and
condition, distinguishes supported effects from sensitivity to the sampling
unit.

This paper makes four contributions. First, it introduces a constituent level
formulation and a crossed protocol over composition, compound context, and
operating condition for compound motor faults whose constituents span
mechanical and electrical modalities, separating novel composition, compound
context deprivation, and operating condition novelty within a single design
(Sections~\ref{sec:methods} to \ref{sec:results}). Second, it develops a
prior aware and mechanism resolved diagnostic evaluation that combines input
independent prevalence references, constituent level ranking and thresholded
performance, learnability controls on isolated faults, and sensitivity to
classifier family. This decomposition helps distinguish empirical patterns
consistent with weak constituent specificity, with sensitivity to classifier
capacity, or with degradation that emerges when an otherwise learnable
constituent is evaluated in compound faults. Third, it presents a controlled
analysis of constituent transfer, spanning oracle detectability within a
pair, transfer across partners, preservation of physical signatures, and a
statistically supported reversal of score orientation, each bounded by
recording sensitivity and same date controls that reduce the risk of reading
variation related to recording as fault physics. Finally, sensitivity and
robustness analyses over the decision threshold objective, over severity
merging for single faults, and over four classifier families show that
absolute decomposition quality is sensitive to operating point and classifier
family, while the recombination advantage over two sided withholding under
unseen conditions remains directionally positive across all four classifier
families.

The principal findings are that closed set discriminability (accuracy 0.913)
coexists with exact constituent set recovery of only 0.083 to 0.157 on unseen
compounds for the locked probe, and that the resulting failures are strongly
mechanism specific rather than uniform. The bearing inner, bearing outer, and
...
discrimination as an isolated fault (AUROC 0.9995 and $F_1=0.923$) to
compound recall of 0.017 and $F_1$ of 0.028. Static eccentricity retains
substantial discriminative information but is strongly sensitive to
classifier family, whereas winding is already less constituent specific in
isolation and remains poor across nonlinear alternatives. Evidence transfers
across partners often, but it can additionally undergo severe reorientation
of scores that depends on context. The primary crossed analysis suggested a
larger penalty from complete compound context deprivation when the operating
condition was also unseen, but this interaction became unresolved after
matching total training set cardinality and is therefore treated as a
directional observation that depends on the sensitivity analysis rather than
as an effect independent of training support.
...
Six research threads intersect in this study. They are compound fault
...
the public datasets that underpin this literature. We review each and then
position the present work (Table~\ref{tab:positioning}).
...
convolutional network whose multilabel classifier recovers the constituents
of gearbox compound faults from raw vibration signals \cite{huang2019ddcnn},
and a one dimensional CNN variant with a multilabel output layer
\cite{huang2019multilabel}. Subsequent work refined the decoupling head with
capsule networks that use dynamic routing and label activation by threshold
\cite{ltssbow2024}, with attentional residual networks combined with active
learning for measured rather than injected bearing compound faults
\cite{decoupling_attn2020}, and with mixture of experts architectures whose
attention is adapted to the task for decoupling compounds unseen as
combinations \cite{hemtan2024}. Two properties are common among the
representative studies considered here. The constituents are
\emph{mechanical}, such as bearing races, balls, and gear teeth, so every
constituent is observable in the same vibration channel. In addition, several
evaluations rely on steady speed laboratory recordings and on protocols
defined at the sample or segment level that do not explicitly test
independence across physical recordings.

A stricter setting withholds all compound data and requires composing single
fault knowledge at test time. Xu \emph{et al.} introduced zero shot compound
fault diagnosis with manually defined semantic attributes learned from single
fault data \cite{eswa2022}, later replacing hand crafted attributes with
fault semantics learned by an autoencoder \cite{eswa2023}. Generative
formulations synthesize pseudo samples of unseen compounds from single fault
semantics \cite{zsl2021,kbs2025}. Graph embeddings propagate relational
structure among base faults \cite{neuro2025}, and semantic spaces informed by
physics anchor the attribute vectors in knowledge of characteristic
frequencies \cite{song2025gzsl}. Reported accuracies on unseen compounds
cluster between roughly 75\% and 87\%
\cite{eswa2022,eswa2023,song2025gzsl,appac2025}.

These results, however, concern compounds whose constituents are homogeneous,
meaning bearing and gear combinations in which both mechanisms modulate the
same vibration carrier. The MCC5-THU motor dataset instead contains compound
compositions that combine bearing defects with broken bar, eccentricity, and
winding mechanisms. Their curated signatures span carrier and imbalance
descriptors in the current domain as well as, for eccentricity, shaft order
descriptors in vibration. The resulting setting therefore tests heterogeneous
multimodal constituent transfer rather than a compound task confined to the
vibration domain. To our knowledge, this cross modal compound regime has not
been evaluated in the zero shot compound fault literature, and no published
constituent level results exist for this dataset.

A parallel literature shows that choices of evaluation protocol, more than
choices of model, drive many reported accuracies near ceiling. Wheat
\emph{et al.} quantified the impact of data leakage within a bearing in
vibration diagnosis and advocated splits from part to part \cite{wheat2024}.
Recent work formalizes the distinction between leakage at the segmentation
level and leakage at the specimen level and proposes leakage safe protocols
...
knowledge, unexplored, yet it is precisely where leakage is most subtle. In
the dataset used here, severity variants of one physical combination can leak
across a composition boundary, while compound and single fault recordings may
also differ in conditions related to recording, which can confound naive
contrasts between compound and single faults. Our protocol design extends
leakage aware evaluation to the compositional setting through run level and
severity aware splits, through recording sensitivity and same date controls,
and through constant predictor references for the principal constituent level
metrics.

Diagnosis under variable speed and load has produced a large literature on
domain adaptation and domain generalization, surveyed in \cite{han2025}. On
the MCC5-THU family specifically, the dataset authors proposed evidential
ensemble learning for multimode diagnosis in real time \cite{liu2024tii} and
recently questioned how operating conditions should enter learning based
diagnosis at all \cite{rethinking2025}. Other groups applied condition robust
architectures to the companion gearbox release \cite{wang2024,phmap2025}. The
dominant protocol in this literature holds one condition out of many
\cite{han2025,domainshift2025}. The present study instead evaluates operating
condition novelty jointly with compound context exposure through the crossed
design over composition, context, and condition.
...
\cite{thomson2001mcsa,nandi2005review,bellini2008advances,mcsa_review2017}.
Deep multimodal fusion typically concatenates modalities into one
representation or fuses learned features, for example multilevel information
fusion \cite{multilevel2019}, multimodal networks with dynamic routing
\cite{drmnn2020}, adaptive CNN fusion of denoised vibration and current
\cite{plos2025}, and comparative studies of the two modalities under deep and
classical learners \cite{ayankoso2026}. These works report that multimodal
fusion can improve conventional single fault diagnosis. In the present study,
constituent detectors instead use predefined feature families that are
specific to each mechanism, restricting each detector to physically relevant
descriptors from either the vibration or the current domain and thereby
reducing direct competition between unrelated modality features.

Public datasets of rotating machinery underpin this literature, among them
CWRU \cite{smith2015cwru} and Paderborn \cite{lessmeier2016} for bearings,
together with more recent releases for motors and drivetrains with multiple
faults such as a PMSM stator fault dataset \cite{jung2023} and an industrial
scale dataset for a motor and pump \cite{bruinsma2024}. The MCC5-THU family,
comprising a gearbox release \cite{chen2024gearbox} and the motor release
used here \cite{chen2026}, is distinctive in combining single and compound
faults, twelve speed and load profiles with transitional segments, and
synchronized channels for vibration, three phase current, torque, and key
phase. The motor release is distributed without constituent level reference
baselines, and the present study provides, to our knowledge, its first
systematic constituent level evaluation.

Table~\ref{tab:positioning} contrasts this study with representative prior
work along the axes developed above. The distinguishing combination has five
elements, namely compound faults whose constituents live in different
modalities, generalization over composition, compound context, and operating
condition varied jointly in one crossed protocol, splits that are run level,
severity aware, and leakage safe, principal constituent level metrics
interpreted against constant predictor references specific to each
constituent so that scores driven by prevalence cannot masquerade as
diagnostic skill, and bootstrap inference clustered by run with multiplicity
control.
...
The study used the public MCC5-THU motor dataset \cite{chen2026}, comprising 288 physical recordings from a 2.2-kW three-phase induction motor across 24 diagnostic states and 12 speed/load operating conditions. The release contains healthy, single-fault, and nine compound-fault compositions, yielding 108 physical compound runs. Synchronized measurements include triaxial vibration, three-phase current, torque, and keyphase signals; the present constituent analysis used vibration, current, and keyphase measurements. Table~\ref{tab:rig} summarizes the machine, instrumentation, and fault-seeding
procedures; all faults are physically seeded rather than simulated, and the
severity levels listed there are the basis of the severity-merging analysis
of Section~\ref{subsec:constituent_detection}. Because the twelve operating
conditions span 1000--3000~rpm, the machine is inverter fed, and the measured
supply frequency accordingly varies with the speed setpoint (16.6, 33.2, and
50.1~Hz at the three nominal speeds).

\begin{table}[!t]
\caption{Test rig, instrumentation, and fault seeding
\cite{chen2026}. Severity levels in parentheses are the released variants.}
\label{tab:rig}
\centering
\footnotesize
...
Property & Value \\
\midrule
Machine & QABP-90L2, 2.2~kW three-phase induction \\
Poles & 2 (one pole pair) \\
Single-side air gap & 0.5~mm \\
Bearings & SKF 6205-2Z-C3, 9 balls, 7.94~mm ball, \\
         & 39.04~mm pitch \\
Bearing orders & BPFO 3.585, BPFI 5.415, BSF 2.357 \\
Drivetrain & torque sensor, two-stage parallel gearbox, \\
           & magnetic powder brake \\
\midrule
Vibration & triaxial accelerometer, drive end, 100~mV/g \\
Current & three current clamps, 100~mV/A \\
Torque & S2001, 100~mV/Nm, $\pm$0.5\%~F.S. \\
Key-phase & one pulse per revolution \\
Sampling & 12.8~kHz, 8 channels, 90~s per run \\
\midrule
Bearing races & laser-cut raceway notches, $\pm$0.01~mm \\
              & (mild, severe) \\
Bearing ball & 360$^\circ$ circumferential pitting on one ball \\
Winding short & shunt resistor across two winding taps, \\
              & circulating current (5\%, 10\% of rated) \\
Broken bar & three consecutive rotor bars removed \\
Static eccentricity & end-shield adjustment screws \\
                    & (0.125~mm, 0.250~mm radial offset) \\
Shaft bending & four-point bending, 0.2~mm permanent bend \\
Voltage unbalance & series resistance in one supply phase \\
                  & (4\%, 8\%) \\
\bottomrule
\end{tabular}
\end{table}

Each experimental run $i$ was represented by a binary constituent vector
...
The fixed $\pm0.12$ half-width was verified against the released signals
rather than adopted by convention. Because a fixed absolute window spans a
smaller relative width at higher harmonics while slip-induced deviation of a
bearing order grows with harmonic number, the observed spectral peak departs
from its kinematic order by a median of $0.024$ and $0.036$ order at the BPFO
and BPFI fundamentals but $0.049$ and $0.080$ order at their second
harmonics; the $\pm0.12$ window consequently contains the observed peak in
96--100\% of single-fault runs at the fundamental and in 74\% at the BPFI
second harmonic. Widening the window does not repair this and degrades
discrimination, because $2 o_{\mathrm{BPFO}}$ and $2 o_{\mathrm{BPFI}}$ lie
close to integer shaft orders that carry rotational energy in healthy and
faulty machines alike: separation of faulty from healthy runs at
$2 o_{\mathrm{BPFO}}$ falls from AUROC $0.877$ at $\pm0.12$ to $0.573$ at
$\pm0.25$ and to chance at $\pm0.50$. The $\pm0.12$ choice is therefore
retained as the better available compromise on this rig, with
second-harmonic descriptors---BPFI's in particular---understood to carry
less reliable evidence than their fundamentals.
...
together with their second harmonics \cite{randall2011tutorial}. Bearing
descriptors were extracted from the Hilbert-envelope vibration order
spectrum. Importantly, the Hilbert
...
broken-bar model of the classical $(1\pm2s)f_e$ form
\cite{thomson2001mcsa,bellini2008advances}. Because these sidebands may also vary with load, operating
...
The distinction is quantitative on this rig. The detected carrier lies at
$50.06$~Hz against a $49.69$~Hz shaft at the 3000~rpm setting, identifying a
two-pole machine, so that $s = 1 - 1/o_e$ yields slip of 3.0\%, 1.5\%, and
1.0\% at the 1000, 2000, and 3000~rpm settings. The classical broken-bar
sidebands at $(1\pm2s)f_e$ therefore sit $2 s\, o_e = 0.062$, $0.031$, and
$0.020$ order from the carrier---about two spectral bins at the analysis
window length used here, and within the $\pm0.12$ band centered on $o_e$
itself in 92.7\% of analyzed windows. That signature is thus absorbed into
the carrier descriptor rather than resolved separately, whereas $o_e\pm1$ and
$o_e\pm2$ lie one and two shaft orders out, coinciding with the $f_e \pm f_r$
spacing associated with eccentricity. Resolving genuine broken-bar sidebands
on this rig would require substantially longer analysis windows or explicit
slip-tracked demodulation.
...
to 0.157 on unseen compounds. The two results are not in tension. They
measure different abilities, and the gap between them is the subject of this
paper.
...
matches. Under recombination, recovery of all constituents reached 0.417 to
0.444, compared with 0.333 for the reported prevalence reference over three
constituents. That reference, however, incurred 1.667 false additions per run
compared with approximately 0.64 to 0.66 for the learned detector. These
results show why constituent recall must be interpreted jointly with false
additions, micro averaged $F_1$, and exact set recovery rather than as a
stand alone measure of diagnostic skill. The constituent resolved controls
...
0.9995 and $F_1=0.923$) but collapsed to compound recall of 0.017 and $F_1$
of 0.028, providing the clearest evidence that poor compound recovery cannot
be explained by weak detectability in isolation alone. Static eccentricity
was likewise highly learnable in isolation and retained substantial compound
ranking information, while its $F_1$ increased from 0.577 with the locked
linear probe to 0.866 with the RBF SVM, consistent with pronounced
sensitivity to classifier family. Winding showed a third pattern, in which
specificity as an isolated fault was already weaker and none of the nonlinear
alternatives rescued its poor compound performance. In contrast, the bearing
inner, bearing outer, and broken bar evidence remained strongly
discriminative across both isolated and compound settings. Constituent level
failure in this dataset therefore exhibits distinct empirical patterns
consistent with compound degradation, with sensitivity to classifier
capacity, and with limited constituent specificity rather than a uniform
cross modal deficit.
...
For the interaction between recombination and two sided withholding,
clustering at the composition level yielded $+0.040$ for micro averaged $F_1$
(95\% CI $[-0.004,\,0.086]$), $+0.051$ for constituent recall
(95\% CI $[0.009,\,0.093]$), and $+0.083$ for recovery of all constituents
(95\% CI $[0.009,\,0.157]$). The primary analysis at the composition level
therefore supported the interaction for constituent recall and for recovery
of all constituents, while the interaction in micro averaged $F_1$ remained
unresolved. After matching the recombination training set cardinality to the
two sided regime, the interactions remained positive on average but were
unresolved for micro averaged $F_1$ ($+0.010$, 95\% CI
$[-0.040,\,0.061]$), for constituent recall ($+0.021$,
$[-0.037,\,0.088]$), and for recovery of all constituents ($+0.031$,
$[-0.074,\,0.157]$). The observed context effect is therefore sensitive to
the available training support and should not be interpreted as an effect of
compound context exposure that is independent of training cardinality.
...
requires that reversal to be interpreted as score transfer that depends on
context rather than as proven inversion of a physical signature.

Fourth, absolute decomposition quality is a property of the operating point
and of the model. Thresholding weighted toward precision raised exact match
recovery into the range 0.157 to 0.287 at reduced recall, and nonlinear
classifier families reduced false additions from approximately 0.68 to
approximately 0.26 per run, while the recombination advantage over two sided
withholding under unseen conditions remained directionally positive across
all four classifier families.
...
demanding in combination than either alone. Because that interaction did not
survive matching of training cardinality, however, this is offered as a
direction for commissioning practice to test rather than as an established
requirement. Several directions remain open. The dataset provides
insufficient replicated recordings of the same state to separate fault,
assembly, sensor mounting, thermal, and recording related effects. The
transfer analyses therefore stop at carefully bounded attribution, and
systematically replicated acquisitions would enable stronger separation of
these potential sources of variation. Representation or normalization
strategies that promote consistency of constituent scores across compound
partners are identified as hypotheses rather than established remedies.
Finally, the constituent level protocol, the prevalence references, and the
controls used here are directly portable to the companion gearbox release and
to other multimodal datasets, where replication across datasets of the
interaction between context and condition would test its generality.
...
recording-sensitivity and same-date controls were recovered directly from the
released file names and therefore require no additional data source to
reconstruct.

\section*{Code Availability}
The code developed and used during the current study is available from the
