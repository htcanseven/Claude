...
classifier families. Finally, it reports reference experiments
(Appendix~\ref{app:reference}) that bound the study's design choices from
outside the constituent-level pipeline: a protocol comparison establishing
that operating-condition robustness must be claimed in the extrapolating
direction, a measurement of what a shared multimodal feature space does to
the weaker constituent, empirical support for two feature-construction
parameters, and documentation of release-level properties that materially
affect protocol design.
...
concludes the paper. Appendix~\ref{app:reference} reports the reference
experiments on protocol difficulty, modality sharing, and feature physics.
...
\url{https://doi.org/10.5281/zenodo.XXXXXXX}. The release includes the
reference experiments of Appendix~\ref{app:reference}, so that the protocol
comparison, the modality-dissociation experiment, and the feature-physics
checks can be reproduced from the public dataset without re-deriving the
preprocessing chain.
