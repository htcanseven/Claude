...
Ablations (--variant)
  full          fusion + adversarial + physics features
...
def run_protocol(name, S, E, Ycomp, ycond, tr, te, variant, args, rows):
    ...
    for seed in args.seeds:
        ...
        dl = DataLoader(TensorDataset(
            torch.from_numpy(Xtr), torch.from_numpy(Etr),
            torch.from_numpy(Ytr.astype(np.float32)),
            torch.from_numpy(ctr.astype(np.int64))),
        ...
        for ep in range(args.epochs):
            ...
            tot = 0.0
            for xb, eb, yb, cb in dl:
                xb, eb, yb, cb = (xb.to(dev), eb.to(dev), yb.to(dev),
                                  cb.to(dev))
                opt.zero_grad()
            ...
            if (ep + 1) % 4 == 0:
                print(f"    s{seed} ep{ep + 1}/{args.epochs} "
                      f"loss={tot / len(Ytr):.4f} lambda={lambd:.2f}",
                      flush=True)
        model.eval()
        outs = []
        with torch.no_grad():
        ...
        P = (np.concatenate(outs) > 0).astype(np.int8)
        ...
        print(f"  {name:30s} {variant:11s} s{seed} exact={exact:.4f} "
              f"microF1={rows[-1]['micro_f1']:.4f}", flush=True)
...
def main() -> int:
    ...
    ap.add_argument("--variants", nargs="+",
                    default=["full", "no_adv", "no_physics", "no_fusion"])
